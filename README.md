...
5. **Permanent regression coverage and failure-pattern rule.** Tag-filtered inventory tables must not combine a parameter-dependent base result with independently refreshed enrichment through parallel or reused client-side merges. Prefer one direct ARG result; collapse every one-to-many child-resource input before joining and collapse the final output by its authoritative ARM ID. Named-query and forbidden-pattern tests enforce the direct-query contract, full-ID cardinality, join-key survival, subscription-scoped workload attribution, optional-enrichment visibility, Capacity's six-join limit and all three shared-RG warning surfaces. The complete local suite now runs 358 tests across 35 suites.
...
A correctness release for the **🏗️ Capacity** tab's Log Analytics **storage, IOPS, latency and network** charts, addressing [#86](https://github.com/Azure/AzureLocal-LENS-Workbook/issues/86). Reported feedback highlighted that a cluster's **Storage Usage (%)** on the *Top 5 Clusters* chart did not reconcile with the actual disk usage seen on that cluster's nodes. Investigation found a shared design flaw across every Perf-counter storage / IO / network chart on the Capacity **Overview**, **Multi-cluster** and **Single cluster** sub-tabs: each query **`union`-ed overlapping counter families** (`LogicalDisk` + `Cluster CSV File System` + `Cluster Shared Volume`) and **`union`-ed `Perf` with `InsightsMetrics`** with no de-duplication, then took a **flat average**. This over-weighted volumes that appear in more than one counter family (and nodes onboarded to both Perf and VM Insights), and — for the storage-usage percentage specifically — averaged tiny OS / **recovery** partitions at **equal weight** with the large data volumes, producing a cluster figure that could not be reconciled against real data-volume utilisation. **All fixes stay within the counters you already collect — no Data Collection Rule (DCR) changes are required.** Changes touch [`workbooks/Capacity-Overview/Capacity-Overview.workbook`](workbooks/Capacity-Overview/Capacity-Overview.workbook), [`workbooks/Capacity-SingleCluster/Capacity-SingleCluster.workbook`](workbooks/Capacity-SingleCluster/Capacity-SingleCluster.workbook) and [`workbooks/Capacity-MultiCluster/Capacity-MultiCluster.workbook`](workbooks/Capacity-MultiCluster/Capacity-MultiCluster.workbook), and were validated against live Log Analytics.

1. **Storage Usage (%) — rebuilt on a de-duplicated, capacity-aware precedence model.** The *Storage Usage (%) — Top 5 Clusters* chart (Capacity **Overview**), the *Storage Usage (%) — By Machine* chart (**Single cluster**) and the **Storage** row of the *Predictive Resource Exhaustion Forecast* table (**Multi-cluster**) previously matched all three disk counter families plus `InsightsMetrics FreeSpaceMB` and averaged every matching row. Because the same Cluster Shared Volume is reported under more than one family — and because only `_Total` / `HarddiskVolume1` were excluded, leaving the OS `C:` volume and small system / **recovery** partitions in the average — the figure blended unlike volumes at equal weight. Each chart now resolves **one value per node per time-bin** using a five-tier precedence: **(1)** `Cluster CSV File System` `% Used Space`; **(2)** `Cluster Shared Volume` `% Used Space`; **(3)** `InsightsMetrics` **capacity-weighted** bytes, reducing repeated samples to one `avg(FreeSpaceMB)` and one `max(diskSizeMB)` per disk before summing across disks, and excluding the OS `C:` mount; **(4)** `LogicalDisk` `% Used` for Cluster Shared Volume mount points (`InstanceName has "ClusterStorage"`); and **(5)** as a last resort, `LogicalDisk` `% Used` across mounted volumes **excluding the bare `HarddiskVolume*` system / recovery partitions**. The best available tier per node and time-bin wins (`arg_min` on the tier rank), so no counter family is counted twice, repeated metadata cannot inflate capacity weighting, and recovery / system partitions no longer inflate the fallback result. Clusters that collect the reference-DCR `Cluster CSV File System` counters report the authoritative data-volume utilisation; other environments fall through the ordered alternatives without losing the chart entirely.

2. **Storage IOPS & Storage Latency — counter-family + source precedence (OS disks retained).** The *Storage IOPS* and *Storage Latency (ms)* charts on all three sub-tabs unioned the same three counter families (and, for IOPS, `InsightsMetrics`), which over-weighted any volume reported under more than one family. Each now selects a **single counter family per node and minute** in precedence order `Cluster CSV File System → Cluster Shared Volume → LogicalDisk`, with `InsightsMetrics` as the final fallback where available. Selecting at minute granularity lets fallback data fill genuine gaps without allowing one preferred sample to suppress the fallback for the entire chart range. Per-volume samples are reduced to one value per physical host per minute (IOPS summed; latency averaged), then to one daily value per host before cluster averages are calculated, so hosts with more volumes no longer receive extra weight. The Overview VM Insights fallback also sums `ReadsPerSecond` + `WritesPerSecond` into total host IOPS. These charts intentionally **retain the OS / system disks** — they measure storage-subsystem *performance*, not data capacity — so only the double-counting and unequal host weighting are removed.

3. **Network Throughput — stop summing two overlapping perf objects.** The *Network Throughput (MB/s)* charts summed `Bytes Total/sec` across **both** the `Network Interface` **and** `Network Adapter` perf objects, which report the same physical NICs — doubling throughput on any node where both objects are collected. Each chart now selects a **single Perf object per node** (`Network Interface` preferred, `Network Adapter` as fallback), then selects `Perf` over `InsightsMetrics` per node and minute. This keeps object choice stable while allowing the fallback source to fill minute-level gaps, so throughput is counted once without blanking otherwise available intervals.
...
5. **Workspace “All” failure guidance across Capacity views.** Capacity workspace picker guidance explains that **All** queries the workspaces in the selected scope and can succeed when they are accessible and resolvable. If Azure rejects the cross-resource scope before KQL begins because a workspace is unreadable, deleted, or cannot be resolved, operators should narrow the picker to workspaces they can read. Non-rendering `value::all` conditional warning items were removed.

6. **DCR deployment guidance — CLI-first, Policy for continuity, portal fallback.** The hidden-by-default Capacity setup guide has been simplified around one recommended path: deploy the dedicated all-in-one [LENS DCR template using the detailed instructions](example-dcr-template/README.md). The template collects all 27 host and Hyper-V counter paths plus Event 3002. Azure CLI covers explicit initial rollout and repair; Azure Policy covers future or rebuilt hosts, with remediation required for existing nodes; portal editing remains the complete manual merge path and now explicitly requires all 27 exact counter paths plus Event 3002. Repeated recommendation text has been removed. The redundant standalone Hyper-V DCR template has also been removed because associating it alongside the all-in-one DCR would overlap all 14 Hyper-V paths. The guidance makes clear that multiple DCR associations are additive but AMA does not deduplicate overlapping sources.
...
An incremental release that bundles several improvements across the workbook — we ship small, focused changes rather than waiting for a single large drop. This cycle: an Azure Resource Graph (ARG) correctness fix plus usability work on the **📋 System Health** tab, a column-label fix and a *View Progress* deep-link fix on **🔄 Update Progress**, a one-sentence purpose summary on every tab (each prefixed **Reporting data** or **Actionable data** so you know at a glance what a tab is for), and clearer, de-duplicated filter/refresh guidance in the shared header. Changes touch [`workbooks/SystemHealth/SystemHealth.workbook`](workbooks/SystemHealth/SystemHealth.workbook), [`workbooks/UpdateProgress/UpdateProgress.workbook`](workbooks/UpdateProgress/UpdateProgress.workbook), the Overview / Machines / VMs / AKS Arc / ARB Status / Capacity tab files, and [`shared/header.json`](shared/header.json) (filter/refresh guidance plus the version-banner bump). The new *Show Succeeded Checks* control is opt-in and defaults to off, so the Detailed Results table behaves identically out of the box; the summary-count fix in item 5 changes results only for very large fleets that were previously under-counted.

1. **System Health — new `Show Succeeded Checks` toggle on the Detailed Results table (default Off).** The *🔍 24 Hour System Health Checks - Detailed Results* grid is intentionally scoped to **failures / non-succeeded** health checks only — its inner Azure Resource Graph query has always carried a hard-coded `| where tostring(healthCheck.status) != "Succeeded"` predicate that runs immediately after the `mv-expand` of each cluster's `healthCheckResult` snapshot. This keeps the view focused on what needs attention and minimises the data returned in large fleets (each cluster can carry hundreds of check entries, the overwhelming majority of which are `Succeeded`). Feedback indicated this design was not discoverable — users seeing an empty or short table could not tell whether their estate was healthy or whether passing checks were simply being hidden. **Fix:** a new single-select `Show Succeeded Checks` pill (`detailShowSucceeded` parameter) is added to the Detailed Results filter row, immediately after the **Check Severity** filter. It defaults to **No (failures / non-succeeded only)** — preserving the prior behaviour byte-for-byte — and offers **Yes (include succeeded checks)** to surface the full check set. The hard-coded status predicate is gated accordingly: `| where '{detailShowSucceeded}' == 'true' or tostring(healthCheck.status) != "Succeeded"`. When the toggle is left at its default the emitted query is logically identical to v1.0.4.

2. **System Health — Detailed Results row limit raised from 2,000 to 10,000.** Because enabling **Show Succeeded Checks** can multiply the returned row count by an order of magnitude in large fleets (succeeded entries are the bulk of every cluster's `healthCheckResult` snapshot), the grid's `gridSettings.rowLimit` is raised from `2000` to `10000` so that turning the toggle on is less likely to silently truncate results. The failures-only default rarely approached the old cap, so this has no practical effect unless the new toggle is enabled.

3. **System Health — Detailed Results columns reordered for triage-first readability, and `Overall Health State` renamed to `Cluster Health Rollup`.** The grid previously led with **Overall Health State** and buried **Check Severity** at column 9 — *after* the wide *Display Name* / *Description* / *Remediation* text columns — even though the table is sorted by severity (`severityOrder`). The `project` clause is reordered so the at-a-glance triage columns appear leftmost and adjacent: **Cluster Name → Check Severity → Health Check - Display Name → Check Result → Health Check - Description → …** (Display Name — *what* the check is — precedes Check Result — *how it did* — which is near-constant `Failed` in the failures-only default view), with the rollup column demoted to sit after the remediation column. That column is also **renamed from `Overall Health State` to `Cluster Health Rollup`** to make explicit that it is a per-cluster value repeated on every row of a given cluster (not a per-check state). The peer cluster-summary table higher on the tab (one row per cluster) keeps its `Overall Health State` label, where the wording is accurate. No data or filter semantics change — only column order and the one label.

4. **System Health — Detailed Results tip reworded to clarify data freshness and document result limits.** The info tip beneath the table previously described the `healthCheckResult` array as *"historical"*, which implied an append-only log. It is reworded to make clear each row is an *individual* check from the cluster's **most recent** health-check evaluation snapshot (each with its own **Timestamp**), and that a since-fixed check only drops off the view after the cluster **re-runs its health evaluation and republishes** results — a periodic / on-demand process that is separate from (and slower than) Azure Resource Graph's own few-minute sync. This explains why a cluster showing a `Success` **Cluster Health Rollup** can still list a non-succeeded check whose **Timestamp** predates the fix. A **Result limits (large fleets)** note is also appended documenting the 10,000-row grid cap and the per-cluster `mv-expand` cap (see item 5).

5. **System Health — health-check summary correctness fix: per-cluster `mv-expand` cap raised from 128 to 2,000.** Azure Resource Graph silently truncates `mv-expand` output to **128 child rows per parent by default** — an undocumented behaviour that caused the *❌ Health Check Failures By Reason Summary* table, the *🥇 Top 5 - System Health Check Issues* chart, and the *Filter by Cluster* picker that feeds them to **silently under-count** failed checks for any cluster reporting more than 128 total health-check entries. Real fleets routinely carry 300–400 entries per cluster, so aggregate `FailureCount` / affected-cluster `dcount` values and the Top-5 ranking could all be understated with no warning. **Fix:** every health-check `mv-expand` in the tab now uses the explicit `limit 2000` form (`| mv-expand healthCheck = healthCheckResult limit 2000`) — `2000` is the ARG maximum — lifting the per-cluster ceiling well above any realistic entry count. This was validated against live Azure Resource Graph. An ℹ️ **Note (large fleets)** callout is also added above the failures summary documenting the 2,000-entry ceiling. *(An initial attempt used `mv-apply` to filter-before-expand, but ARG does not support `mv-apply`; the `mv-expand … limit` form is the correct ARG-native approach.)*

6. **System Health — Update Readiness Summary clarified and given a fleet-total row.** Feedback flagged the *Update Readiness Summary* matrix as hard to interpret. Three changes address this without altering the underlying per-cluster data: **(a)** the warning callout above the table is rewritten to explain the matrix layout — each cluster appears **once**, rows group clusters by **Update Status**, columns break each row down by **Health State**, and the **Total** column (already present) is the cluster count for that status. **(b)** A **grand-total row** labelled *📊 All clusters (total)* is appended, giving an at-a-glance fleet count per health state; it is produced ARG-natively by duplicating each row into its own bucket plus an "All Clusters" bucket via `mv-expand` before the `summarize` (a `union` total leg was rejected by ARG's *one remote-table reference per query* rule, so the `mv-expand` rollup is used instead, and was validated against live ARG). **(c)** The redundant grid `title` is dropped, since the rewritten callout now serves as the heading.

7. **Update Progress — *Clusters with Updates Available* table: final column relabelled `resourceGroup` → `Resource Group`.** The *📦 Clusters with Updates Available* grid surfaced its last column under the raw Azure Resource Graph property name `resourceGroup`, the only column on that table without a friendly `labelSettings` override. A `labelSettings` entry mapping `resourceGroup` → **Resource Group** is added, matching the existing **Resource Group** column on the *Update Run History* table elsewhere on the same tab. Display-only change — no query or data semantics affected.

8. **All tabs (and the four Capacity sub-tabs) — one-sentence purpose summary under each heading, prefixed with a `Reporting data` / `Actionable data` label.** Each tab repeats its name at the top of its content area, but several gave no indication of *what the tab is for* before the reader hit a dense grid — and the tabs differ in intent. Every top-level tab, plus each of the four **Capacity** sub-tabs, now carries a single orienting sentence led by a bold **Reporting data:** or **Actionable data:** prefix classifying it by its primary value (where a tab mixes both, it takes the higher-value **Actionable** label rather than listing both). The split:
   - **Reporting data** (inventory / planning) — *Overview*, *VMs*.
   - **Actionable data** (surfaces you visit to find and fix something) — *Machines* (⚠️ Non-Connected Machines and ⚠️ Failed Node Extensions, with full error detail), *AKS Arc Clusters* (⚠️ Failed AKS Extensions, ⚠️ Non-Compliant Flux Configurations, 🔐 Certificate Expiration Warning), *System Health*, *Update Progress*, *ARB Status*, and **all four Capacity sub-tabs**: *Overview* (Log Analytics **Top 5** highest-utilisation clusters), *Multi-cluster* (resource-exhaustion forecasts for clusters approaching limits), *Single cluster* (drill into one cluster to spot a host pinned at high CPU / memory / storage), and *Hyper-V VMs* (**Top VMs / disks** by CPU, memory, storage, IOPS, latency, network).
   - The parent **Capacity** tab keeps only its navigation tip (no label of its own), since its four sub-tabs each carry their own classification.

   The System Health sentence is also reworded from the awkward *"Triage surface for…"* noun-stack to a plain sentence. Text-only change; no query, filter, or layout semantics affected.

9. **Shared header — misleading `Last Refreshed` tab line removed; **Time Range** and **Refresh** guidance consolidated into the *Filter Instructions* panel.** Every tab header carried the line *"**Last Refreshed:** {TimeRange:label} | Click refresh on any tile to update data"*. The label was misleading: `{TimeRange:label}` is the value of the workbook's **Time Range** filter (the time window applied to time-based tiles), **not** a timestamp of when data was last loaded — so a default of *"Last 45 days"* read as if the data were 45 days stale. Rather than repeat a corrected version of this line on all six tabs, it is **removed from the per-tab headers entirely** and replaced once, at the top of the workbook, with two new bullets in the shared *Filter Instructions* panel ([`shared/header.json`](shared/header.json)): a **Time Range** bullet explaining it sets the time window for time-based trend/history tiles (and that point-in-time inventory tiles always show current state), and a **🔄 Refresh data** bullet giving explicit guidance — *"Click the ↻ Refresh button in the workbook toolbar to re-run all queries (or hover over a single tile and click its refresh icon to reload just that tile)."* Text-only change; no query or filter semantics affected.

10. **Update Progress — *Clusters Currently Updating* table: *View Progress* link now deep-links to the in-flight update run.** The **View Progress** link in the *⏳ Clusters Currently Updating* table was built with the generic `/updateName~/null/updateRunName~/null/` portal form, which made the Azure portal default to a *stale or failed* update run rather than the update actually in progress. **Fix:** the update-run name is now captured from the update-run resource `id` *before* the query's `summarize … by clusterId` (which drops the `id` column) and carried through `arg_max`, so the link can embed the specific `updateName` and `updateRunName` — mirroring the working deep-link already used by the *Update Run History* table on the same tab. Link-target-only change in [`workbooks/UpdateProgress/UpdateProgress.workbook`](workbooks/UpdateProgress/UpdateProgress.workbook); no data, filter, or layout semantics affected.

The workbook header banner bumps from `v1.0.4` to `v1.0.5`.

### v1.0.4

A targeted hardening release that strengthens cluster-selection precision on the **🏗️ Capacity** tab when an environment contains multiple Azure Local clusters that happen to share the same short `name` across different resource groups or subscriptions. All changes are backwards-compatible from a UX perspective — single-cluster-named tenants see no behavioural difference; tenants with name collisions stop seeing cross-cluster data fan-out on the **🔍 Single Cluster** drill-down and stop seeing collapsed-into-one-row fan-out on the **🌍 Multi-cluster** *Predictive Resource Exhaustion Forecast* table.

1. **Capacity → Single Cluster (renamed from Single Node) — ARM-id alignment hardening.** Six Azure Resource Graph (ARG) `where` clauses on the **🔍 Single Cluster** drill-down tab — the *Clusters* tile, the *Nodes* tile, the *Node Hardware Summary* table, the *Storage Volume Usage* bar-chart join, the *Hyper-V VM Inventory* join and the *AKS Arc Clusters* join — previously filtered the `microsoft.azurestackhci/clusters` table with a name-only predicate: `| where name =~ tostring(split('{SingleCluster}', '/')[8])`. That predicate slices the selected cluster's ARM resource id at the eighth `/`-separated segment (which is the cluster's short `name`) and compares it against the `name` column of every cluster resource visible under the current `{Subscriptions}` + `{ResourceGroupFilter}` + `{ClusterTagName}/{ClusterTagValue}` scope — so in tenants that happened to have two or more Azure Local clusters with the same short name across different resource groups (or different subscriptions), all of them matched and their node counts, hardware rows, storage volumes, Hyper-V VM inventories and AKS Arc clusters were silently unioned together into the per-cluster drill-down. The lower per-machine **Performance**, **Storage** and **Network** charts on the same tab were *not* affected because they already keyed off `armId =~ '{SingleCluster}'` via the `ClusterNodeMap` shared parameter — which is itself a strong indicator that the bug existed in only one direction (the upper ARG tiles showed inflated cluster-wide counts while the lower Log Analytics charts correctly reported `no results` for nodes that didn't exist on the *actually-selected* cluster). **Fix:** all six predicates now match by ARM resource id directly — `| where id =~ '{SingleCluster}'` — bringing the ARG-side scoping into alignment with the LA-side scoping that was already in place. Same-named clusters now stay isolated end-to-end across the tab.

2. **Capacity → Single Cluster — new `Resource Group` dropdown + enriched cluster picker labels.** A new optional `SingleClusterResourceGroup` multi-select pill is inserted between the **Log Analytics Workspace** picker and the **Select Cluster** picker on the **🔍 Single Cluster** tab. It defaults to *All* and is populated from `distinct resourceGroup` of all `microsoft.azurestackhci/clusters` visible under the current global `{Subscriptions}` + `{ResourceGroupFilter}` + `{ClusterTagName}/{ClusterTagValue}` scope. When one or more resource groups are selected, the **Select Cluster** picker query is narrowed accordingly, making it trivial to disambiguate same-named clusters without leaving the tab. The **Select Cluster** picker's `label` projection is also enriched from plain `name` to `strcat(name, ' (', resourceGroup, ')')` so each entry shows its resource-group qualifier at-a-glance even with the default *All* resource-group filter. The enriched label automatically flows through every `{SingleCluster:label}` interpolation already in the tab's section headers (*"Capacity — `<cluster> (<rg>)`"*, *"Storage Volume Usage (GB) — `<cluster> (<rg>)`"*, *"Hyper-V VM Inventory — `<cluster> (<rg>)`"*, *"AKS Arc Clusters — `<cluster> (<rg>)`"*) so the active context is unambiguous regardless of how the user got to the tab.

3. **Capacity → Multi-cluster — Forecast filter ARM-id alignment hardening.** The *Cluster* dropdown on the **📉 Predictive Resource Exhaustion Forecast by Cluster** table (driven by the `ForecastClusterFilter` parameter) previously emitted plain cluster names as picker values (`project value = name, label = name`) and the consumer query filtered with `| where clusterName in (split('{ForecastClusterFilter}', ','))`. Identical in shape to issue #1, same-named clusters across different resource groups collapsed into a single row of the forecast table — and selecting any one entry in the picker filter applied to *all* of them. The peer **📊 Cluster-wise Resource Forecast** chart filter (`ChartClusterFilter`, used by the four cluster-wise CPU / Memory / Storage / per-volume-storage forecast charts immediately below the table) already keyed off ARM resource id and was unaffected. **Fix:** the picker now emits `value = tostring(id), label = strcat(name, ' (', resourceGroup, ')')` (matching the Single Cluster picker's convention); the consumer query is extended to enrich its `nodeMap` materialisation with the `armId` column already carried in the `ClusterNodeMap` shared parameter (`parts[2]` of each colon-delimited entry), to compute an `armIdByCluster = nodeMap | distinct clusterName, armId` lookup, to `leftouter`-join that lookup onto the final per-cluster row set, and to rework the filter clause to `| where ('{ForecastClusterFilter}' == 'value::all' or ArmId in~ (split('{ForecastClusterFilter}', ',')))`. The chart-side filter (`ChartClusterFilter`) is unchanged — it has been ARM-id-based since v1.0.1; this fix simply brings the *table*-side picker into the same convention.

4. **Workbooks renamed `Capacity-SingleNode` → `Capacity-SingleCluster` and `Capacity-MultiNode` → `Capacity-MultiCluster` (folder + file slug).** The user-facing tab labels (*🌍 Multi-cluster*, *🔍 Single cluster*), the conditional-visibility values (`multi`, `single`), the inner item-name prefixes (`mc-*` for multi-cluster items, `single-cluster-*` for single-cluster items) and the gallery template folders (`LENS-Capacity-Multi-cluster`, `LENS-Capacity-Single-Cluster`) were already aligned with the *"multi cluster"* / *"single cluster"* phrasing — only the on-disk folder and file slugs still carried the legacy *"MultiNode"* / *"SingleNode"* names from the original split-from-monolithic refactor in v1.0.0 (where Azure Local's *Node Capacity* dashboard heritage was the prevailing terminology). The slugs now read `workbooks/Capacity-MultiCluster/Capacity-MultiCluster.workbook` and `workbooks/Capacity-SingleCluster/Capacity-SingleCluster.workbook` throughout: [`scripts/template-ids.json`](scripts/template-ids.json), [`scripts/split-capacity.js`](scripts/split-capacity.js), [`scripts/build-gallery.js`](scripts/build-gallery.js) (comment), [`CONTRIBUTING.md`](CONTRIBUTING.md) and the monolithic-structure table in this README are all updated. Both git renames are recorded as `R` moves so file history is preserved end-to-end. **No customer-visible impact**: the assembled monolithic JSON ([`AzureLocal-LENS-Workbook.json`](AzureLocal-LENS-Workbook.json)) is structurally identical regardless of where its source lived on disk — only contributors editing the per-tab sources see the new paths.

5. **Multi-cluster-in-RG attribution hardening across `Overview`, `VMs`, `AksArc`, `Capacity-SingleCluster` and `Capacity-Overview`.** When two or more Azure Local clusters are deployed into the *same* Azure resource group (a supported but uncommon topology), the Azure Resource Graph (ARG) join chain that LENS uses to attribute workloads back to their owning HCI cluster — `microsoft.azurestackhci/virtualmachineinstances` / `microsoft.hybridcontainerservice/provisionedclusterinstances` / `microsoft.azurestackhci/storagecontainers` → `extendedLocation.name` → `microsoft.extendedlocation/customlocations` (joined by extracting `arcBridgeRG` from `properties.hostResourceId`) → `microsoft.azurestackhci/clusters` (joined by `resourceGroup`) — collapses on `resourceGroup` at the final hop, because ARG exposes no deterministic ARB-appliance → HCI-cluster mapping (no name convention, no tag cross-reference, no `extendedLocation` on the cluster resource). The cartesian join produced `N × M` row fan-out across every workload row, inflating VM and AKS-Arc counts by exactly the number of sibling clusters in the shared RG. This release lands two complementary mitigations:
   - **Deduplication via `dcount(childId)` and `make_set` / `strcat_array` collapse** on every workload-counting tile, chart and table where the count *can* be made correct from the post-fan-out row set. The pattern (blessed in `ArbStatus.workbook` v0.8.9) replaces `count()` with `dcount(childId)` for tiles, `take_any(field) by childId` for charts/tables, and `strcat_array(make_set(field), ', ') by parentId` for any field that needs comma-collapse back to a parent. Applied across **2 sites in [`workbooks/Overview/Overview.workbook`](workbooks/Overview/Overview.workbook)** (the *Total VMs* and *Total AKS Arc* count tiles), **9 sites in [`workbooks/VMs/VMs.workbook`](workbooks/VMs/VMs.workbook)** (VM inventory, status pie, OS pie, generation pie, power state pie, vCPU/memory tiles, joins) and **8 sites in [`workbooks/AksArc/AksArc.workbook`](workbooks/AksArc/AksArc.workbook)** (cluster count, control-plane / agent-pool aggregations, K8s version pie, status grids, joins). Tile / chart / table counts now report the correct N rather than N × M.
   - **Transparent warning banners + sibling-disclosure column** on the **🏗️ Capacity** tab and its **🔍 Single Cluster** drill-down, for the storage-attribution and capacity-aggregation queries where the join chain still produces RG-level (not cluster-level) totals that *cannot* be disambiguated post-hoc: storage container capacity (`storageUsedGB` / `storageAvailGB` / `storageUsedPct`), per-cluster VM vCPU / memory roll-up (`vmVCPUTotal` / `vmMemoryTotalGB`) and per-cluster AKS Arc vCPU roll-up (`aksVCPUTotal`). A new hidden parameter `MultiClusterRGShared` (in [`workbooks/Capacity-Overview/Capacity-Overview.workbook`](workbooks/Capacity-Overview/Capacity-Overview.workbook), inside the `cap-shared-params` block) issues a single ARG query that returns `'yes'` when any RG in the current `{Subscriptions}` + `{ResourceGroupFilter}` + `{ClusterTagName}/{ClusterTagValue}` scope contains 2+ HCI clusters, and an empty string otherwise. When set to `'yes'` it lights up a `style: warning` banner immediately above the *Cluster Capacity Overview* table that names the affected columns and explains why their totals are RG-level rather than cluster-level. The Cluster Capacity Overview table itself gains a new **RG Attribution** column (`rgAttribution`), computed (without burning an additional ARG join — the table already runs at the 6-join cap) by `union`ing a sibling-cluster summary (`summarize rgClusterNames = make_set(name), rgClusterCount = count() by tolower(resourceGroup)`) into the existing workload subquery's `summarize ... by arcBridgeRG` step, then collapsing the two unioned rows per RG via an outer `summarize` that takes `sum()` of the workload metrics and `take_any` / `max` of the cluster aggregates. A per-row `set_difference(rgClusterNames, pack_array(name))` then lists the sibling cluster names sharing the row's RG (empty string when the cluster is the sole occupant of its RG). The peer **🔍 Single Cluster** drill-down already received the same treatment in an earlier checkpoint of this release: a hidden `MultiClusterRGSiblings` parameter feeds three context-specific warning banners on the *Storage Volume Usage*, *Hyper-V VM Inventory* and *AKS Arc Clusters* sections so the operator sees the disclosure once per affected section. **Design rationale:** an earlier prototype attempted proportional splitting (allocating each storage volume's `storageUsedGB` to sibling clusters by relative `storageTotalMB` weighting) — this was reverted on user direction because the proportional weights have no causal relationship to which sibling cluster actually owns each volume, and presenting a precise-looking but fabricated number is more misleading than a clearly-flagged RG-level total. The transparent-warning approach is consistent with how `ArbStatus.workbook` handles the same root-cause join-chain weakness, and gives operators an explicit lever (deploy each Azure Local cluster into its own resource group) to eliminate the warning altogether at the deployment-pattern level.

6. **Capacity → Multi-cluster — Top-5 cluster charts and "Azure Local Machines:" coverage tiles tightened to physical-host-only filtering.** The six **Top 5 Clusters** charts immediately below the **🔔 Performance Counter DCR Configuration** section — *CPU Usage (%)*, *Memory Usage (%)*, *Storage Usage (%)*, *Storage Latency (ms)*, *Storage IOPS* and *Network Throughput (MB/s)* — together with the six paired **`Azure Local Machines: N`** coverage tiles that sit directly beneath them, all derive their per-cluster aggregates by filtering Log Analytics `Heartbeat` / `Perf` / `InsightsMetrics` rows down to the set of physical Azure Local hosts. Prior to this release only the **Memory Usage (%)** chart used the strict filter that the `ClusterNodeMap` shared parameter makes available — the other five charts and *all six* coverage tiles fell back to a looser predicate that kept any Windows machine whose `Heartbeat.ResourceGroup` happened to match an HCI cluster's resource group. In tenants that route a single DCR (Data Collection Rule) to *every* Arc-enrolled Windows machine in a cluster's RG — for example, when the same DCR is intentionally associated with the **Azure Local VMs** (Hyper-V Windows guests, Arc-enrolled via the Connected Machine agent) running on top of the cluster, in addition to the physical hosts themselves — the coverage tiles then over-counted (reporting e.g. *62* "Azure Local Machines" when the cluster has only 14 physical hosts, with the remaining 48 being Arc-enrolled VM guests in the same RG sending the same `\Memory\Committed Bytes` / `\Memory\Available Bytes` / `\Processor(_Total)\% Processor Time` / `\LogicalDisk(C:)\% Free Space` / `\Network Adapter(*)\Bytes Total/sec` counters that the DCR collects), and the five looser charts mixed guest-VM workload into the cluster-level *Top 5* averages (skewing CPU%, LogicalDisk%, network MB/s and storage IOPS / latency away from the physical-host signal the tab is trying to surface). **Fix:** all five looser charts plus all six coverage tiles now use the exact filter pattern that the Memory chart already had — a `materialize`-d `hciNodes` CTE that first `lookup`s `Heartbeat.ResourceGroup` against the `{ClusterRGMap}` shared parameter (the "is this RG one we recognise?" predicate, preserved as a fallback) and then `join`s on the machine's short hostname against the `{ClusterNodeMap}` shared parameter (a `nodeShort:clusterName` map of the *actual* HCI host short names per cluster, populated from ARG's `microsoft.azurestackhci/clusters | properties.reportedProperties.nodes` reflection). The clause `| where hasNodeMap == false or nmCluster == clusterName` keeps the broad RG-only behaviour for clusters whose node map is empty (genuinely disconnected clusters that have not yet reported any nodes to ARG) and restricts to exact-host-name matches everywhere else — so the coverage tiles now report the precise physical-host count and the Top-5 charts now compute per-cluster averages from physical-host samples only. The pattern was already proven on the Memory chart in v1.0.0; this release simply extends it to the remaining 11 sibling queries on the same sub-tab.

No other workbook content changes. The workbook header banner bumps from `v1.0.3` to `v1.0.4`.

### v1.0.3

A small post-v1.0.2 patch driven by a customer-reported gap in the **🔄 Update Progress** tab's *📜 Update Run History and Error Details* table, where a real failed update on an `HealthCheckSucceeded` progressStatus was silently filtered out by the table's *Status* dropdown and (after that fix) duplicated by a stale dedup key. Both fixes live exclusively in [`workbooks/UpdateProgress/UpdateProgress.workbook`](workbooks/UpdateProgress/UpdateProgress.workbook).
...
| &nbsp;&nbsp;↳ [workbooks/Capacity-MultiCluster/Capacity-MultiCluster.workbook](workbooks/Capacity-MultiCluster/Capacity-MultiCluster.workbook) |   ↳ 🌍 Capacity — Multi-cluster | 60 KB |
| &nbsp;&nbsp;↳ [workbooks/Capacity-SingleCluster/Capacity-SingleCluster.workbook](workbooks/Capacity-SingleCluster/Capacity-SingleCluster.workbook) |   ↳ 🔍 Capacity — Single cluster | 93 KB |
