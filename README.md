...
4. **Memory Usage (%) — Top 5 Clusters chart** rewritten to match Cluster Insights (`(Total Physical − Available) / Total Physical`) instead of the misleading `% Committed Bytes In Use` counter, which used Commit Limit (physical RAM + page file) as its denominator and consistently under-reported usage.
...
**Root cause:** LENS was averaging the raw `\Memory\% Committed Bytes In Use` performance counter, whose Windows-defined formula is `Committed Bytes / Commit Limit × 100`, where `Commit Limit = Physical RAM + Page File Size`. On Hyper-V hosts and Azure Local nodes the page file is large by default, so the denominator is much bigger than physical memory and the resulting percentage is always lower than what users (and Cluster Insights) consider "memory usage". Cluster Insights instead uses `(Total Physical Memory − \Memory\Available Bytes) / Total Physical Memory × 100`, which is what users intuitively read as "memory usage".

**Fix — three coordinated changes:**

- **Counter swap.** The Memory Usage chart KQL now collects `\Memory\Available Bytes` (a stock Windows perfmon counter) instead of `\Memory\% Committed Bytes In Use`. The DCR setup guide on the Capacity — Overview tab has been updated to recommend this counter; the noDataMessage on the chart has been updated to reference it. Customers upgrading from v1.0.0 should add `\Memory\Available Bytes` to their existing DCR — `\Memory\% Committed Bytes In Use` is no longer required by any LENS chart and can be removed (or left in place; it is harmless).
- **New shared parameter `ClusterMemMap`.** A hidden ARG-backed parameter (analogous to the existing `ClusterRGMap`) maps each cluster name to its total physical memory in GiB, derived from `properties.reportedProperties.nodes[0].memoryInGiB × nodeCount` on `microsoft.azurestackhci/clusters`. This avoids any dependency on the SDDC Management event log (which the upstream Insights workbook uses) and means the chart works for any LENS-instrumented cluster without further configuration. The parameter is added to `shared/parameters.json` and propagated to all 12 sub-templates by `scripts/sync-shared-params.js`.
- **Per-node, per-cluster aggregation.** The chart KQL now averages `Available Bytes` per node per time bin, then sums across nodes within each cluster, and divides by the cluster's total physical bytes from `ClusterMemMap`. This matches Insights' `(Total − Available) / Total × 100` formula closely (Insights does it per-node; LENS does it per-cluster, which is correct for a multi-cluster Top-5 chart). The result is floored at 0 with `max_of(…, 0.0)` so transient `Available > Total` reporting glitches do not produce negative percentages.
