...
**Fix:** lowercase both sides — `indexof(tolower(id), "/providers/microsoft.azurestackhci")` etc. The outer `tolower(...)` wrapper that canonicalized the substring result is unchanged, so semantics are preserved for already-working clusters; only the previously-dropped rows recover. **21 occurrences swept across 5 workbook source files** (Capacity-Overview, Capacity-SingleNode, ARB Status, Overview/Instances, AKS Arc Clusters). The **Azure Local VMs** tab is not affected by this bug — although it is also ARG-sourced, its tiles query `microsoft.hybridcompute/machines | where kind == "HCI"` directly and join to `microsoft.azurestackhci/clusters` via shared resource group (`vmResourceGroup == clusterResourceGroup`), so they never execute the buggy `indexof(id, "/providers/Microsoft.AzureStackHCI")` substring extraction against the `extensibilityresources` companion records at all. Verified by 225/225 tests passing post-sweep with no other content drift.
