...
8. **All tabs (and the four Capacity sub-tabs) — one-sentence purpose summary under each heading, prefixed with a `Reporting data` / `Actionable data` label.** Each tab repeats its name at the top of its content area, but several gave no indication of *what the tab is for* before the reader hit a dense grid — and the tabs differ in intent. Every top-level tab, plus each of the four **Capacity** sub-tabs, now carries a single orienting sentence led by a bold **Reporting data:** or **Actionable data:** prefix classifying it by its primary value (where a tab mixes both, it takes the higher-value **Actionable** label rather than listing both). The split:
   - **Reporting data** (inventory / planning / single-target inspection) — *Overview*, *Machines*, *VMs*, *AKS Arc Clusters*, and the *Capacity → Single cluster* sub-tab.
   - **Actionable data** (surfaces you visit to find and fix something) — *System Health*, *Update Progress*, *ARB Status*, and three of the four Capacity sub-tabs: *Overview* (Log Analytics **Top 5** highest-utilisation clusters), *Multi-cluster* (resource-exhaustion forecasts for clusters approaching limits), and *Hyper-V VMs* (**Top VMs / disks** by CPU, memory, storage, IOPS, latency, network).
   - The parent **Capacity** tab is therefore labelled **Actionable data** (three of its four sub-tabs surface hot-spots), even though it also carries planning/trend reporting.
