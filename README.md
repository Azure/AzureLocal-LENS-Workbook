...
A focused **📋 System Health** tab release that combines usability improvements with an Azure Resource Graph (ARG) correctness fix, plus a small **🔄 Update Progress** column-label fix. Changes live in [`workbooks/SystemHealth/SystemHealth.workbook`](workbooks/SystemHealth/SystemHealth.workbook) and [`workbooks/UpdateProgress/UpdateProgress.workbook`](workbooks/UpdateProgress/UpdateProgress.workbook) (plus the version-banner bump in [`shared/header.json`](shared/header.json)). The new *Show Succeeded Checks* control is opt-in and defaults to off, so the Detailed Results table behaves identically out of the box; the summary-count fix in item 5 changes results only for very large fleets that were previously under-counted.
