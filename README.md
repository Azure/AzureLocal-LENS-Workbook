...
4. **Workbook renamed `Capacity-SingleNode` → `Capacity-SingleCluster` (folder + file slug).** The user-facing tab label (*🔍 Single cluster*), the conditional-visibility value (`single`), the inner item names (`single-cluster-*`) and the gallery template folder (`LENS-Capacity-Single-Cluster`) were already aligned with the *"single cluster"* phrasing — only the on-disk folder and file slug still used the legacy *"SingleNode"* name carried over from the original split-from-monolithic refactor in v1.0.0. The slug now reads `workbooks/Capacity-SingleCluster/Capacity-SingleCluster.workbook` throughout: [`scripts/template-ids.json`](scripts/template-ids.json), [`scripts/split-capacity.js`](scripts/split-capacity.js), [`scripts/build-gallery.js`](scripts/build-gallery.js) (comment), [`CONTRIBUTING.md`](CONTRIBUTING.md) and the monolithic-structure table in this README are all updated. The git rename is recorded as a single `R` move so the file's history is preserved end-to-end. **No customer-visible impact**: the assembled monolithic JSON ([`AzureLocal-LENS-Workbook.json`](AzureLocal-LENS-Workbook.json)) is structurally identical regardless of where its source lived on disk — only contributors editing the per-tab sources see the new path.
