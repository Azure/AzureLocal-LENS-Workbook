...
No workbook-functional changes in this patch beyond the embedded ARM template's counter list. The standalone **🖥️ Hyper-V VMs** sub-tab continues to ship its own dedicated, scoped ARM template for customers who want Hyper-V telemetry **only** (i.e. without the host counters). The workbook header banner bumps from `v1.0.1` to `v1.0.2`.
