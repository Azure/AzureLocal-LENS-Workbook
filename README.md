...
5. **Multi-cluster-in-RG attribution hardening across `Overview`, `VMs`, `AksArc`, `Capacity-SingleCluster` and `Capacity-Overview`.** When two or more Azure Local clusters are deployed into the *same* Azure resource group (a supported but uncommon topology), the Azure Resource Graph (ARG) join chain that LENS uses to attribute workloads back to their owning HCI cluster — `microsoft.azurestackhci/virtualmachineinstances` / `microsoft.hybridcontainerservice/provisionedclusterinstances` / `microsoft.azurestackhci/storagecontainers` → `extendedLocation.name` → `microsoft.extendedlocation/customlocations` (joined by extracting `arcBridgeRG` from `properties.hostResourceId`) → `microsoft.azurestackhci/clusters` (joined by `resourceGroup`) — collapses on `resourceGroup` at the final hop, because ARG exposes no deterministic ARB-appliance → HCI-cluster mapping (no name convention, no tag cross-reference, no `extendedLocation` on the cluster resource). The cartesian join produced `N × M` row fan-out across every workload row, inflating VM and AKS-Arc counts by exactly the number of sibling clusters in the shared RG. This release lands two complementary mitigations:
   ...
   - **Transparent warning banners + sibling-disclosure column** on the **🏗️ Capacity** tab and its **🔍 Single Cluster** drill-down, for the storage-attribution and capacity-aggregation queries where the join chain still produces RG-level (not cluster-level) totals that *cannot* be disambiguated post-hoc: storage container capacity (`storageUsedGB` / `storageAvailGB` / `storageUsedPct`), per-cluster VM vCPU / memory roll-up (`vmVCPUTotal` / `vmMemoryTotalGB`) and per-cluster AKS Arc vCPU roll-up (`aksVCPUTotal`). A new hidden parameter `MultiClusterRGShared` (in [`workbooks/Capacity-Overview/Capacity-Overview.workbook`](workbooks/Capacity-Overview/Capacity-Overview.workbook), inside the `cap-shared-params` block) issues a single ARG query that returns `'yes'` when any RG in the current `{Subscriptions}` + `{ResourceGroupFilter}` + `{ClusterTagName}/{ClusterTagValue}` scope contains 2+ HCI clusters, and an empty string otherwise. When set to `'yes'` it lights up a `style: warning` banner immediately above the *Cluster Capacity Overview* table that names the affected columns and explains why their totals are RG-level rather than cluster-level. The Cluster Capacity Overview table itself gains a new **RG Attribution** column (`rgAttribution`), computed via a `leftouter` self-join on `microsoft.azurestackhci/clusters` with `summarize rgClusterNames = make_set(name), rgClusterCount = count() by rgSibKey = tolower(resourceGroup)` and a per-row `set_difference(rgClusterNames, pack_array(name))` to list the sibling cluster names sharing the row's RG (empty string when the cluster is the sole occupant of its RG). The peer **🔍 Single Cluster** drill-down already received the same treatment in an earlier checkpoint of this release: a hidden `MultiClusterRGSiblings` parameter feeds three context-specific warning banners on the *Storage Volume Usage*, *Hyper-V VM Inventory* and *AKS Arc Clusters* sections so the operator sees the disclosure once per affected section. **Design rationale:** an earlier prototype attempted proportional splitting (allocating each storage volume's `storageUsedGB` to sibling clusters by relative `storageTotalMB` weighting) — this was reverted on user direction because the proportional weights have no causal relationship to which sibling cluster actually owns each volume, and presenting a precise-looking but fabricated number is more misleading than a clearly-flagged RG-level total. The transparent-warning approach is consistent with how `ArbStatus.workbook` handles the same root-cause join-chain weakness, and gives operators an explicit lever (deploy each Azure Local cluster into its own resource group) to eliminate the warning altogether at the deployment-pattern level.
