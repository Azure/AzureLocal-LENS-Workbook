...
A focused **📋 System Health** tab release that combines usability improvements with an Azure Resource Graph (ARG) correctness fix, plus a small **🔄 Update Progress** column-label fix and one-sentence purpose summaries across all tabs. Changes live in [`workbooks/SystemHealth/SystemHealth.workbook`](workbooks/SystemHealth/SystemHealth.workbook) and [`workbooks/UpdateProgress/UpdateProgress.workbook`](workbooks/UpdateProgress/UpdateProgress.workbook), with the tab-summary additions also touching the Overview, AKS Arc, and ARB Status tab files (plus the version-banner bump in [`shared/header.json`](shared/header.json)). The new *Show Succeeded Checks* control is opt-in and defaults to off, so the Detailed Results table behaves identically out of the box; the summary-count fix in item 5 changes results only for very large fleets that were previously under-counted.
...
9. **All tabs — `Last Refreshed` header line relabelled `Time period` and the refresh hint clarified.** Every tab header carried the line *"**Last Refreshed:** {TimeRange:label} | Click refresh on any tile to update data"*. The label was misleading: `{TimeRange:label}` is the value of the workbook's **Time Range** filter (the time window applied to time-based tiles), **not** a timestamp of when data was last loaded — so a default of *"Last 45 days"* read as if the data were 45 days stale. It is relabelled **Time period** and annotated to say it is set with the **Time Range** filter at the top of the workbook and applies to time-based tiles. The vague *"Click refresh on any tile to update data"* is replaced with explicit guidance: *"use the ↻ **Refresh** button in the workbook toolbar (or hover a tile and click its refresh icon)."* Text-only change across all six affected tab headers; no query or filter semantics affected.
