...
A focused **📋 System Health** tab release that combines usability improvements with an Azure Resource Graph (ARG) correctness fix, plus a small **🔄 Update Progress** column-label fix, one-sentence purpose summaries across all tabs, and clearer shared filter/refresh guidance. Changes live in [`workbooks/SystemHealth/SystemHealth.workbook`](workbooks/SystemHealth/SystemHealth.workbook) and [`workbooks/UpdateProgress/UpdateProgress.workbook`](workbooks/UpdateProgress/UpdateProgress.workbook), with the tab-summary additions also touching the Overview, AKS Arc, and ARB Status tab files, and the shared filter/refresh guidance plus version-banner bump in [`shared/header.json`](shared/header.json). The new *Show Succeeded Checks* control is opt-in and defaults to off, so the Detailed Results table behaves identically out of the box; the summary-count fix in item 5 changes results only for very large fleets that were previously under-counted.
...
8. **All tabs — one-sentence purpose summary under each tab heading.** Each tab repeats its name at the top of its content area, but several gave no indication of *what the tab is for* before the reader hit a dense grid — and the tabs differ in intent (some are operational/triage surfaces, others are inventory or capacity-planning reports). Four tabs already carried a short descriptive line under their heading (**Machines**, **VMs**, **Update Progress**, and **Capacity**'s navigation note); the four that did not — **Overview** (*Azure Local Instances*), **AKS Arc Clusters**, **System Health**, and **ARB Status** — now each gain a single orienting sentence under their heading, matching the existing in-header description style. Text-only change; no query, filter, or layout semantics affected.
