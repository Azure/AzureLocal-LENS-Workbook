...
8. **All tabs — one-sentence purpose summary under each tab heading, prefixed with a `Reporting data` / `Actionable data` label.** Each tab repeats its name at the top of its content area, but several gave no indication of *what the tab is for* before the reader hit a dense grid — and the tabs differ in intent. Every tab now carries a single orienting sentence under its heading, each led by a bold **Reporting data:** or **Actionable data:** prefix classifying the tab by its primary value (where a tab mixes both, it takes the higher-value **Actionable** label rather than listing both). The split: **Reporting data** — *Overview*, *Capacity*, *Machines*, *VMs*, *AKS Arc Clusters* (inventory / planning surfaces); **Actionable data** — *System Health*, *Update Progress*, *ARB Status* (surfaces you visit to find and fix something). Four tabs already had a descriptive line (Machines, VMs, Update Progress, Capacity's navigation note); the rest — Overview, AKS Arc, System Health, ARB Status — gain one. The System Health sentence is also reworded from the awkward *"Triage surface for…"* noun-stack to a plain sentence. Text-only change; no query, filter, or layout semantics affected.
